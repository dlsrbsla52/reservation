...
///   StopServiceClient가 이 인터페이스를 위임받아 비즈니스 로직을 처리한다.
@HttpExchange(url = "/api/v1/internal/stop")
public interface StopApi {

    /// pk(UUID) 기준 정류소 조회
    @GetExchange
    StopPageResponse getStopByPk(@RequestParam("pk") UUID pk);

    /// stopId(정류소 번호) 기준 정류소 조회
    @GetExchange
    StopPageResponse getStopByStopId(@RequestParam("stopId") String stopId);
}
