...
data class CreateAdminMemberRequest(
    ...
    @param:Schema(description = "로그인 아이디 (4자 이상 50자 이하, 영문/숫자/언더스코어만 허용)", example = "admin_user01")
    ...
    @param:Schema(description = "비밀번호 (영문, 숫자, 특수문자를 각각 1개 이상 포함한 8자 이상)", example = "AdminPass123!")
    ...
    @param:Schema(description = "이메일 주소", example = "admin@example.com")
    ...
    @param:Schema(description = "어드민 회원 유형 (ADMIN_USER | ADMIN_MASTER | ADMIN_DEVELOPER)", example = "ADMIN_USER")
    @field:NotNull(message = "회원 유형을 선택해주세요.")
    val memberType: MemberType,

    @param:Schema(description = "회원 이름", example = "홍길동")
    @param:NotBlank(message = "이름을 입력해주세요.")
