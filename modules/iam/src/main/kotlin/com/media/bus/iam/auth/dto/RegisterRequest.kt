...
data class RegisterRequest(
    ...
    @param:Schema(description = "로그인 아이디 (4자 이상 50자 이하, 영문/숫자/언더스코어만 허용)", example = "user123")
    ...
    @param:Schema(description = "비밀번호 (영문, 숫자, 특수문자를 각각 1개 이상 포함한 8자 이상)", example = "Password123!")
    ...
    @param:Schema(description = "사용자 이메일 주소", example = "user@example.com")
    @field:NotBlank(message = "이메일을 입력해주세요.")
    @field:Email(message = "올바른 이메일 형식을 입력해주세요.")
    val email: String,

    @param:Schema(description = "핸드폰 번호 (하이픈 제외, 숫자만)", example = "01012345678")
    @field:NotBlank(message = "핸드폰 번호를 입력해주세요.")
    @field:Pattern(regexp = "^01[016789]\\d{7,8}$", message = "올바른 핸드폰 번호를 입력해주세요. (예: 01012345678)")
    val phoneNumber: String,

    @param:Schema(description = "회원 유형 (MEMBER 또는 BUSINESS)", example = "MEMBER")
    @field:NotNull(message = "회원 유형을 선택해주세요.")
    val memberType: MemberType,

    @param:Schema(description = "비즈니스 회원 전용 사업자등록번호", example = "123-45-67890", nullable = true)
