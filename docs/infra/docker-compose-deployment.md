...
이 문서는 EC2 같은 단일 서버에서 PostgreSQL, Valkey와 4개 MSA 애플리케이션을 Docker Compose로 실행하는 기준을 정리한다.
...
| 파일 | 용도 |
|------|------|
| `docker-compose.yml` | 단일 서버 운영 배포용. DB/Valkey/하위 서비스는 내부망 전용, Gateway만 공개 |
...
현재 Compose에는 프론트엔드가 없다. 향후 Next.js 프론트엔드를 추가할 경우에는 Gateway와 같은 EC2에서 각각 노출할 수 있다.
...
별도 로그 수집 스택은 두지 않는다. 모든 프로파일이 사람이 읽기 쉬운 패턴 로그를 출력하므로 다음 명령으로 확인한다.

```bash
docker compose logs -f --tail=200 gateway-service
docker compose logs -f --tail=200 stop-service
```

## 운영 주의사항

- 서버 보안 그룹 또는 방화벽에서는 기본적으로 `8080`만 열고, PostgreSQL/Valkey/하위 서비스 포트는 열지 않는다.
- `postgres_data`, `valkey_data`는 Docker named volume으로 유지된다. 데이터를 지울 때만 `docker compose down -v`를 사용한다.
